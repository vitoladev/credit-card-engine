...
- Keep the repo's glossary terms (`CONTEXT.md` or equivalent) and any
  ...
  plain-word those into synonyms.
- Sources of truth are whatever the repo's `AGENTS.md` / `CLAUDE.md`
  names; the tracker holds tasks, not product essay.
- Positive stack prose: state the current stack only (no X-not-Y naming
  dead hosts in overview docs). Alternatives belong in ADR Rejected lists.
- Apply `/unslop` after this skill on the same prose. Unslop owns the AI-pattern catalog.
- House style keeps intentional em dashes as separators in some docs;
  prefer a period or new sentence for new prose. Do not strip house-style
  separators while "Global English" would replace every em dash.
- Skill / agent markdown under `.agents/skills/` and `.cursor/agents/` is
  in scope for this skill whenever those files change.
...
- Apply repo `/unslop` (`.agents/skills/unslop`) to every doc this skill touches. That skill owns the slop-pattern catalog: AI vocabulary, filler, hedging, formatting tells.
